...
import { propertyExists, createProperty, logCron, updateSourceStats, resetSourceDetailsCounters, resetDetailsFetched } from './strapi-client';
    ...
    try {
      ...
      if (parser.fetchDetails) {
        ...
        if (req.documentId) {
          ...
          await resetDetailsFetched(req.documentId).catch(() => {});
