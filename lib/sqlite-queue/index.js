...
class SqliteQueue {
    ...
    requestCancellation(id) {
        const now = Date.now();
        if (this.stmtCancelPending.run(now, id).changes > 0)
            return true;
    ...
    clearQueue(queue) {
        ...
        const clear = this.db.transaction(() => {
            const pending = this.db.prepare("UPDATE jobs SET status = 'failed', error = 'queue cleared', completed_at = ? WHERE queue = ? AND status = 'pending'").run(now, queue).changes;
