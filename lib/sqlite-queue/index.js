...
class SqliteQueue {
    ...
    `);
        ...
        this.stmtFindByIdempotency = this.db.prepare(`SELECT * FROM jobs WHERE queue = ? AND idempotency_key = ? AND status IN ('pending', 'active') LIMIT 1`);
    ...
    add(queue, data, opts) {
        ...
        for (let attempt = 0; attempt < MAX_IDEMPOTENCY_CONFLICT_RETRIES; attempt++) {
            try {
                const result = this.stmtAdd.run(queue, JSON.stringify(data), correlationId, idempotencyKey, now, scheduledAt, priority, maxAttempts);
                ...
                lastConflict = err;
                const active = this.stmtFindByIdempotency.get(queue, idempotencyKey);
                if (active)
                    return this.rowToJob(active);
    ...
    process(queue, handler, opts) {
        ...
        const worker = new worker_1.Worker(this.db, queue, handler, {
            ...
            pollInterval,
            leaseDurationMs: opts?.leaseDurationMs ?? this.staleTimeoutMin * 60000,
    ...
    heartbeat(jobId, leaseToken) {
        const now = Date.now();
        const legacyLeaseDurationMs = this.staleTimeoutMin * 60000;
    ...
    requestCancellation(id) {
        const now = Date.now();
        if (this.stmtCancelPending.run(now, now, id).changes > 0)
            return true;
    ...
    clearQueue(queue) {
        ...
        const clear = this.db.transaction(() => {
            const pending = this.db.prepare("UPDATE jobs SET status = 'failed', error = 'queue cleared', completed_at = ?, cancellation_requested_at = COALESCE(cancellation_requested_at, ?) WHERE queue = ? AND status = 'pending'").run(now, now, queue).changes;
            const active = this.db.prepare("UPDATE jobs SET cancellation_requested_at = COALESCE(cancellation_requested_at, ?) WHERE queue = ? AND status = 'active'").run(now, queue).changes;
