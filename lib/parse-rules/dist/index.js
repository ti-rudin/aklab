...
exports.regionFromStructuredLocation = regionFromStructuredLocation;
...
function regionFromStructuredLocation(location) {
    const canonicalRegion = location.region?.trim().toLocaleLowerCase('ru-RU');
    if (canonicalRegion && exports.REGION_VALUES.includes(canonicalRegion)) {
    ...
    if (/башкортостан/u.test(structured) || code === '02')
        return exports.Region.other;
    if (['50', '90', '150', '190', '750', '790'].includes(code))
        return exports.Region.mo;
    if (code === '77' || /(?:^|[\s,.;])москв(?:а|е|ой)(?=$|[\s,.;])/u.test(structured)) {
        return exports.Region.moscow;
    }
    if (/московск(?:ая|ой)\s+област|\bмо\b/u.test(structured))
        return exports.Region.mo;
    const tverOblast = /тверск(?:ая|ой)\s+област|\bтверская\b/u.test(structured) || code === '69';
    if (tverOblast) {
        return /(?:^|[\s,.;])(?:г\.?\s*)?тверь(?=$|[\s,.;])/u.test(address)
            ? exports.Region.tver
            : exports.Region.tver_oblast;
    }
    if (/(?:^|[\s,.;])(?:г\.?\s*)?тверь(?=$|[\s,.;])/u.test(structured))
        return exports.Region.tver;
    return exports.Region.other;
}
...
function matchesRentFilter(filterRent, candidate) {
