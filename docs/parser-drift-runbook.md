...
6. `detail_supported=false` is an explicit listing-only contract: a successful bounded listing sample may be `healthy` with `details_attempted=0`; it must not synthesize a failed detail phase. Canary results carry exact safe-integer `details_attempted/details_ok/details_failed` counters (`details_ok + details_failed = details_attempted`), with listing-only `0/0/0`; legacy `detail_ok` is derived from `details_failed === 0`. Adapters with a real detail method remain fail-closed: HTTP/navigation/extraction failure is thrown and cannot hydrate stale listing data as a successful detail result.
7. A probe deadline is cooperative: timeout requests cancellation but the queue handler does not report terminal completion before its current bounded adapter operation settles and cleanup runs. The orchestrator waits a bounded cancellation-ack window; if owned jobs remain active, it returns `{ run_id, skipped: true, reason: 'terminal_ack_pending', pending_job_ids, results: [] }`, keeps `cancelling/canary`, writes no health, and does not release the lifecycle owner. Pending IDs are safe numeric IDs sorted ascending.
...
- healthy/degraded evidence from canary or a stale normal run does not auto-release quarantine. Every health recording first re-reads current state and uses a conditional compare-and-set; current hard/unknown/null state is held (unknown/null is normalized to `blocked`), held writes preserve status/streak and omit incoming schema fingerprint/reason metadata, and a CAS loser cannot annotate the current run row, overwrite quarantine, or emit an alert/recovery. A reviewed operational state change and explicit approval are required before resumed parse/reparse/deploy.
