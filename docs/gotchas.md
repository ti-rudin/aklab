...
98. **Torgi transient request не должен перезапускать весь scan с нуля** — официальный API периодически отвечает `429/5xx` или даёт network timeout; один такой ответ раньше немедленно валил source, после чего queue до трёх раз повторяла все search queries/pages целиком. Для list и detail API используется одинаковый bounded request retry: максимум 3 attempts, 30s timeout на attempt, backoff `1s/2s`; response body отменяется перед retry. `429/5xx/network` ретраятся, но `401/403/451` и прочие permanent 4xx возвращаются сразу и остаются fail-closed через `parserHttpError`. TLS не отключать: production parser обязан иметь существующий `NODE_EXTRA_CA_CERTS=/usr/local/share/ca-certificates/russian-ca-chain.pem`; проверять только boolean/path existence и live HTTP metadata, не выводить env secrets.
