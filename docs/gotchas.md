...
78. **pipeline:daily вместо per-source crons** (v1.1.59) — удалено 10 per-source parse crons (Source.schedule), analyze cron (08:00), digest:morning cron. Один `pipeline:daily` — проверяет каждый час, запускается в digest_time из settings, mode='full' (parseAll → analyze → digest). `rescheduleSource` — no-op. Оставлен только `cleanup:old` (03:00). Причина: per-source crons не синхронизировали фазы (Phase 1/2), digest мог прийти до завершения парсинга всех источников.
