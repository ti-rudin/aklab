# AKLAB — Strapi 5 Gotchas

> Извлечено из docs/compact-doc.md. Нумерация сохранена.

## Production deploy — rollback backup должен принадлежать текущему invocation

Нельзя восстанавливать generic DB backup: файл может быть создан старым deploy и откатить актуальную production DB. До arm `ERR` handler создать unique WAL-safe backup текущего invocation, проверить `PRAGMA integrity_check`, сохранить его path в runtime variable и разрешать DB restore только при непустом verified path. Ошибка clean-tree/Git preflight до backup не должна трогать DB. ERR handler обязан завершаться исходным non-zero code даже после успешного rollback.

Operational `.env`/DB backups нельзя сохранять внутри Git worktree: immutable preflight закономерно увидит untracked file и остановит deploy. Использовать отдельный backup root вне worktree с mode `600/700`.

## Multi-user migration — legacy relation drift и SMTP ambiguity

`DATA_RELATION_DRIFT` нельзя bypass-ить. Сначала агрегатно определить тип drift без вывода пользовательских данных. Legacy comments без valid property relation невозможно безопасно привязать к пользователю: сделать full DB backup, сохранить полные rows/link tables в отдельный quarantine SQLite mode `600`, удалить только доказанные orphan rows транзакционно и повторить audit.

`user_profiles.digest_email` поддерживает один или несколько получателей через запятую. Поле должно быть Strapi `text`, а не `email`: browser и Strapi `email` отвергают корректный CSV ещё до серверной нормализации. На фронте и API триммить адреса, отклонять пустые/некорректные сегменты, ограничивать список 10 адресами и дедуплицировать без учёта регистра; хранить каноническую строку через `, `. Internal delivery projection передаёт массив, а digest worker отправляет отдельную копию каждому адресу, не раскрывая соседних recipients в `To`.

## Hotfix — digest freshness + SQLite timestamp (v1.1.72)

`GET /api/properties/focus` использует raw SQLite query и возвращает `first_seen_at` как epoch milliseconds, хотя REST path даёт ISO. Digest должен принимать оба корректных представления; ISO-only проверка отбрасывает все свежие focus-объекты и пишет `No focus properties` при ненулевом `undervalued_count`. Не менять порог как workaround: добавить RED-тест с epoch timestamp, затем безопасно валидировать `positive safe integer → Date` и сохранить фильтры old/future.

## Hotfix — parser identity upsert + SQLite JSON (v1.1.71)

`strapi.db.query().create()` обходит REST JSON-transform Strapi. Если передать raw array в JSON-колонку (`tags`, `photo_urls`), `better-sqlite3` выдаёт `SQLite3 can only bind numbers, strings, bigints, buffers, and null`, а parser получает `POST /properties/upsert → 500`. Перед `repository.create()` сериализовать JSON-поля: `tags: JSON.stringify([])`, `photo_urls: JSON.stringify(photo_urls)`. Покрыть это unit-тестом; иначе валидные московские объекты могут пройти фильтры, но не попасть в БД.

## Истёкшие торги — явная дата, а не дата публикации

Удалять объект можно только по явно извлечённому `auction_end_at`, никогда по `published_at_source` или `created_at`: старое объявление может описывать ещё активные торги. Дедлайн нормализуется из формата площадки в UTC; если источник не отдал однозначную дату, запись не удаляется этим cleanup. Отклонённый пользователем объект сохраняется в БД для identity dedup, но обычные list/focus запросы исключают его, пока не запрошен `status=rejected` явно.

## Strapi 5 — gotchas (для новой сессии)

1. **Структура v5 layout** (отличается от v4):
   `api/<name>/content-types/<name>/schema.json` + `controllers/<name>.ts` +
   `services/<name>.ts`. НЕ `api/<name>/schema.json` напрямую.
2. **`routes/<name>.ts` НЕ создавать без причины** — если файл существует,
   Strapi 5 **заменяет** авто-CRUD на твой массив routes. Если создаёшь —
   прописывай **все 5 CRUD-методов** руками. У нас routes/ удалены → авто-CRUD.
3. **dev-mode hot-reload НЕ подхватывает новые content-types в router** —
   после `rm -rf api/.tmp/data.db` или добавления нового content-type
   нужен `rm -rf dist` + production build (`strapi build`) + `strapi start`.
   `strapi develop` недостаточен.
4. **Strapi 5 пропускает `bootstrap()` без admin-юзера**: при первом
   старте (или после сброса БД) Strapi **не вызывает** наш
   `register()`/`bootstrap()`. Создание admin: либо через /admin
   web UI, либо через наш `seedAdmin` (читает .env), либо **SQL-инъекцией**
   в `up_users` (dev-only fallback).
5. **dist/src/index.js пустой после `tsc`** — это нормально, Strapi в
   dev-mode загружает TypeScript напрямую через ts-node, а не из dist.
   Но production build (`strapi build`) — собирает в dist по-настоящему.
6. **Custom controllers require path** — `require('../../services/queueService')`
   из `api/src/api/cron/controllers/cron.ts` НЕ работает в prod, т.к.
   dist-структура: `dist/src/api/cron/controllers/cron.js` → нужен путь
   `../../../services/queueService` (3 уровня вверх до `dist/src/`, затем
   `services/`). Проверять `node -e "require(...)"` на сервере.
7. **Fabrikant.ru — data-slot selectors** (НЕ CSS class selectors!):
   - Карточки: `[data-slot="card"][data-id]`
   - Title: `[data-slot="anchor"]` (первый A-элемент)
   - Цена: `[data-slot="text"]` содержащий "RUB"
   - Номер: `[data-slot="text"]` matching `/^\d+-\d+$/`
   - URL: `/procedure/search/sales` (вкладка "Продажи"), NOT `/procedure/search`
   - Пагинация: `?page=N`
8. **torgi.gov.ru — чистый JSON API** без авторизации:
   `GET /new/api/public/lotcards/search?lotStatus=PUBLISHED,APPLICATIONS_SUBMISSION&size=100`
   Параметр `subjectRFCode` НЕ работает как фильтр → фильтровать в коде.
   Площадь: `characteristics[].code === "totalAreaRealty"`.
9. **Fedresurs Qrator anti-bot** — обход через curl_cffi Python + `/qauth` endpoint:
   - Playwright/curl → 403 (TLS fingerprinting). curl_cffi с `impersonate="chrome124"` → 200
   - `/qauth` → ставит `qrator_ssid2` cookie → REST API работает
   - PoW: MD5(challenge+nonce).startswith("00") — не нужно решать, /qauth обходит
   - **Rate limit (HTTP 451)** — нужна пауза 3-6 сек между запросами + прокси
   - Python subprocess вызывается из Node.js через execFile
   - Порты: health=1357, deploy через deploy-prod.sh
10. **Deploy: immutable exact SHA** — release (version, root
    `package-lock.json`, changelog) готовится и commit/push-ится до SSH,
    обычно workflow `deploy-prod.yml`. `deploy-prod.sh --ref <SHA>` требует
    branch `main`, clean `git status --porcelain` и exact совпадение SHA с
    `origin/main`; затем делает только fast-forward. Скрипт **никогда не
    stash-ит, reset-ит, commit-ит или push-ит tracked-файлы**. Dirty tree —
    fail-closed: снять `git status`/diff, выяснить источник и исправить его
    отдельным Git commit, не применять `checkout -- .` вслепую.
11. **Vite и public/ файлы** — Vite копирует `app/public/` → `app/dist/`
    при build. Changelog обязан быть уже в release commit до build; deploy
    копирует этот commit-артефакт в dist только после успешного health.
    Нельзя генерировать `changelog.json` на сервере после build.
13. **Strapi 5 custom routes + auth** — если заменить `createCoreRouter`
    на explicit routes с `config: { auth: {} }`, Users-Permissions
    требует явного grant для КАЖДОГО action. Без этого — 500 на всех
    endpoints. **Решение**: `createCoreRouter` для CRUD + отдельный
    файл `routes/health.ts` с `auth: false` для custom endpoints.
14. **Seeder идемпотентен** — при добавлении новых полей в schema
    существующие записи НЕ обновляются. Обновлять через API:
    `PUT /api/sources/:documentId { data: { schedule: "0 3 * * *" } }`.
15. **Changelog: русский язык + МСК** — генератор берёт текст из
    git-коммитов (английский). Словарь переводов в
    `scripts/generate-changelog.js` (TRANSLATIONS). Дата/время —
    `TZ=Europe/Moscow` + русские названия месяцев.
16. **Strapi 5 REST API — только documentId** — `GET/PUT/DELETE
    /api/sources/:id` принимает ТОЛЬКО `documentId` (строка вида
    `bv610wntnoe3l25gt3c6yd4t`), НЕ числовой `id`. `entityService`
    возвращает оба поля, но REST — только `documentId`. Баг v1.0.18:
    счётчики total_found/total_created/parse_count = 0, т.к.
    `updateSourceStats` передавал числовой id → 404 → молчаливый
    провал. Исправлено в v1.0.19.
17. **Seeder singleton через entityService** — `entityService.findMany`
    для singleton content-types (Setting) может не находить записи
    из-за draft/published state → seeder создаёт дубли при каждом
    bootstrap. **Решение**: использовать `db.query(uid).findOne({})`
    вместо `entityService.findMany`. Исправлено в v1.0.20.
18. **torgi.gov.ru API limitations** — параметр `size` игнорируется
    (всегда 10 на страницу), `subjectRFCode` не работает как фильтр,
    date параметры тоже игнорируются. Решение: увеличить MAX_PAGES,
    фильтровать в коде по `createDate` и `subjectRFCode`.
19. **Generic parsers** — invest-mosreg, investmoscow, roseltorg, m-ets
    использовали универсальные CSS-селекторы (`.card`, `[class*="card"]`).
    **Обновлено (02.07.2026):** investmoscow переписан на Nuxt SSR (__NUXT_DATA__),
    invest-mosreg на JSON API (/aapi/map/places), m-ets на SPA scraping.
    roseltorg отключён (WAF).
20. **Analyzer numeric id → documentId** — `analyzeAll` в cron controller
    отправлял `property_id: prop.id` (numeric) в очередь, а analyzer
    делал `GET /api/properties/${numericId}` → 404 (REST v5 принимает
    только documentId). Исправлено: `documentId: prop.documentId` +
    `fetchProperty(documentId: string)`. Три файла: cron controller,
    analyzer handler, analyzer strapi-client.
21. **Parser dist не собирается при deploy** — если новый парсер добавлен
    в `services/` но его нет в `scripts/deploy-prod.sh` списке build,
    `dist/` не создаётся → MODULE_NOT_FOUND при старте PM2. Проверять:
    `ls services/parser-*/dist/index.js` на сервере после deploy.
22. **Digest smtp_to fallback** — cron controller читает Setting через
    `entityService.findMany` (singleton, gotcha #17). Если не находит —
    `smtpTo = undefined` → digest использует `config.smtp.user`
    (tirobots@yandex.ru) вместо `Setting.smtp_to` (a@rudin.ru).
    Решение: использовать `db.query` как в seeder, или передавать
    smtpTo напрямую из БД.
23. **PM2 daemon Node version mismatch** — `pm2 report` содержит отдельные
    секции Daemon и CLI и может напечатать две строки `node version`.
    Использовать только section-aware `parse_pm2_daemon_node_version` из
    `scripts/lib/deploy-git-preflight.sh`; `grep` без section boundary склеивает
    версии и даёт ложный mismatch. Production deploy не должен автоматически
    выполнять `pm2 update`: mismatch — fail-closed prerequisite для отдельного
    согласованного maintenance action. Симптом реального mismatch:
    `better-sqlite3 NODE_MODULE_VERSION mismatch` при рестарте.
24. **API_TOKEN_SALT: .env ≠ PM2 daemon** — `access_key` в
    `strapi_api_tokens` = `HMAC-SHA512(plain_token, API_TOKEN_SALT)`.
    Если соль в `.env` отличается от PM2 daemon env — все сервисы
    получают 401. **НЕ** менять `.env` API_TOKEN_SALT вручную —
    PM2 daemon env = source of truth. Deploy-prod.sh синхронизирует.
25. **property_events.property_id** — в Strapi 5 manyToOne relation
    создаёт FK-колонку НЕ автоматически при миграции schema. Нужен
    `ALTER TABLE property_events ADD COLUMN property_id INTEGER` +
    индекс. Без этого raw SQL INSERT в focusEngine падает с
    "table has no column named property_id".
26. **torgi-gov API v2 price fields** — `priceMin`/`priceMax` на верхнем
    уровне объекта, НЕ в `priceInfo.startPrice`. API v2 изменил структуру.
27. **sberbank-ast XML structure** — Playwright загружает динамический XML
    в `input#xmlData` с корнем `<datarow><hits><_source>`. Теги:
    `purchName`, `purchAmount`, `GeoDataAddress`, `Latitude`, `Longitude`,
    `bidHrefTerm`, `BranchNameNew`. НЕ `PurchaseName`/`Amount`/`row`.
28. **investmoscow Nuxt SSR** — `__NUXT_DATA__` массив с обратными ссылками.
    Tender-объекты имеют `startPrice`, `objectArea`, `address` как числовые
    индексы. `coords` может быть невалидным — проверять `Array.isArray`
    и `typeof coords[0] === 'number'`.
29. **isCommercialProperty — доверять парсеру** — если `property_type` уже
    офис/склад/ритейл/производство/free_purpose/apartment → pass, не
    фильтровать по ключевым словам в title.
30. **3-фазный парсинг с fetchDetails** — parse-handler разбит на 3 фазы:
    - Фаза 1: `parser.parse(depth)` → массив properties
    - Фаза 2a: existence check → собираем `newProperties[]` (только новые)
    - Фаза 2b: fetchDetails + createProperty для каждого нового объекта
    `total_details_needed` вычисляется ОДИН РАЗ после phase 2a (= `newProperties.length`),
    `total_details_fetched` инкрементируется после каждого успешного fetchDetails.
    UI показывает `X/Y детальных` где Y фиксировано, X растёт.
31. **resetSourceDetailsCounters** — вызывается в начале КАЖДОГО parse-handler
    (для каждого источника). Обнуляет `total_details_fetched` и `total_details_needed`
    в 0 через PUT `/api/sources/:documentId`. Без этого счётчики кумулятивные.
    **ВАЖНО (v1.1.7):** Промежуточные fire-and-forget `updateSourceStats` УДАЛЕНЫ.
    `total_details_fetched` и `total_details_needed` пишутся ОДИН раз в конце
    (awaited). Stale PUT от предыдущего запуска мог arrive ПОСЛЕ bulk reset и
    перезаписать `total_details_fetched` → fetched > needed.
32. **JWT_SECRET в .env на сервере** — если JWT_SECRET отсутствует в `api/.env`,
    каждый restart API генерирует новую соль → все JWT инвалидируются → 500 Forbidden.
    Symptom: frontend редиректит на /auth. Fix: добавить `JWT_SECRET=<random>` в `.env`.
33. **Strapi 5 возвращает 500 вместо 401** — при невалидном JWT Strapi 5 может
    вернуть 500 + "Forbidden" вместо 401. Response interceptor на фронте должен
    ловить 500 + "Forbidden" и редиректить на /auth.
34. **detectCity — общая функция** — вынесена в `_shared/src/city-detect.ts`,
    используется всеми 7 парсерами. Раньше у каждого парсера была своя копия
    с одинаковым багом. Экспортируется из `@aklab/service-shared`.
35. **detectCity regex: «Москва-Кашира»** — `lower.includes('москва')` матчит
    название дороги «Москва-Кашира» → false positive → `city=moscow` для
    астраханского объекта. Фикс: regex `/(^|[\s,.])москва([\s,.)]|$)/i` —
    дефис НЕ в character class, поэтому «москва-кашира» не матчится.
    Баг: m-ets.ru показывает регион продавца (часто «г. Москва»), а не
    объекта. detectCity теперь проверяет title + description, а не region.
36. **Setting.parse_depth** — глубина парсинга по расписанию (cron). Дефолт 20,
    макс 5000. Cron читает из Setting и передаёт `depth` в `addToQueue()`.
    Поле в schema + UI на `/settings`. Отдельно от ручного запуска (свой ввод).
37. **setting.update permission для authenticated** — seeder НЕ добавлял
    `api::setting.setting.update` → PUT /api/setting возвращал 500 Forbidden
    при сохранении настроек. Fix: добавить в seeder + вручную в БД через
    `up_permissions` + `up_permissions_role_lnk`.
38. **replace_all=true ОПАСЕН для Vue** — `patch(..., replace_all: true)` в
    Vue-файлах с повторяющимися элементами (select/option) может затронуть
    НЕ тот элемент. Пример: замена option в селекте «Тип недвижимости»
    затронула селект «Город», т.к. оба содержат `value="office"`. Всегда
    проверять контекст вокруг заменяемого текста.
39. **property_type enum: apartment + land** — `property/schema.json`
    enum = `['apartment', 'commercial', 'land']`. `apartment` отображается
    как «Квартира», `land` как «Зем. участок». Добавлено в v1.0.102.
40. **classifyPropertyType DRY** — единая функция в
    `_shared/property-classifier.ts`. Все 10 парсеров импортируют оттуда.
    Раньше у каждого парсера была своя копия с расхождениями.
41. **Price/area filter — и парсинг, и анализ** — `price_from`/`price_to` и
    `area_from`/`area_to` в Setting фильтруют И парсинг (в `createProperty()`),
    И анализ. Раньше фильтровали только анализ. С v1.1.13 ParseRules передаётся
    из pipeline/cron через queue job data в parse-handler → createProperty().
42. **propertyExists fail-closed** — при non-OK ответе API (500, 401, etc.)
    `propertyExists` возвращает `true` (skip). Раньше возвращала `false`
    → при 500 от API парсер создавал дубликаты. Баг: 1241 дубликат из 3407.
43. **digest_enabled** — boolean в Setting, default true. 3 уровня защиты:
    cron/index.ts, cron controller, digest handler. `data !== false` для
    обратной совместимости (null/undefined → true).
44. **Pipeline singleton update requires id** — Strapi 5 `update()` для singleton
    content-types (Setting) требует `documentId` в where clause. Без него —
    "Update requires a where parameter" (500). Pipeline state обновляется через
    `db.query(uid).update({ where: { documentId }, data: { pipeline_state } })`.
45. **SSE reconnect при refresh** — pipeline state хранится в БД (не в памяти).
    При перезагрузке страницы фронтенд читает `/api/pipeline/status` → если
    `status === 'running'`, автоматически подключает SSE stream и показывает
    прогресс. Без персистентного state reconnect был бы невозможен.
46. **total_details_needed/fetched — единый awaited write** — оба счётчика
    пишутся ОДИН раз в конце parse-handler (awaited). Промежуточные
    fire-and-forget PUT'ы удалены в v1.1.7 — stale PUT от предыдущего запуска
    мог arrive ПОСЛЕ bulk reset и перезаписать fetched → fetched > needed.
    Bulk reset pipeline перед enqueue оставлен как дополнительная защита.
47. **ParseRules — rules не утекает в Strapi POST** — `createProperty()`
    деструктурирует `rules` из props (`const { rules, ...propertyData } = props`)
    и отправляет только `propertyData` в POST body. Без этого `rules` утекал
    бы в Strapi API payload (Strapi 5 молча игнорирует, но это data integrity issue).
48. **buildParseRules — DRY helper** — единая функция в `api/src/services/parseRules.ts`
    (для pipeline/cron) и `_shared/src/strapi-client.ts` (для парсеров). Дублирование
    из-за того что `api/` не зависит от `@aklab/service-shared`.

49. **vi.resetAllMocks() clears mockImplementation** — если в `beforeEach`
    вызывается `vi.resetAllMocks()`, то `mockImplementation`, заданный на
    уровне модуля, будет сброшен. Нужно восстанавливать его в `beforeEach`.
    Симптом: тесты падают с "X is not a function" только при запуске suite.
50. **Mock call order при нескольких вызовах одного мока** — если код вызывает
    `db.query().findMany()` дважды за итерацию (properties + currentProps),
    то `mockResolvedValueOnce` должны идти в ТОЧНОМ порядке вызовов.
51. **app/ не в npm workspaces** — `npm install` в корне НЕ ставит зависимости
    из `app/`. Для app-specific пакетов нужно `cd app && npm install` отдельно.
52. **services.json порт app** — порт был `4173`, фактически запускался на `5174`.
    Health-check показывал false positive offline. Проверять при изменении ecosystem.
53. **Pipeline POST routes — auth: false** — `POST /pipeline/start|cancel|reset`
    имеют `auth: false` (как GET /status и /stream). Фронтенд не шлёт JWT
    на pipeline endpoints — без `auth: false` POST запросы возвращали 500
    Forbidden. **Исторически** mutating routes (cron, pipeline) имели
    `config: {}` (JWT required), но pipeline — это UI-driven workflow
    без чувствительных данных, поэтому auth не нужен.
54. **Strapi 5 db.query.count()** — может отсутствовать. Безопасный паттерн:
    `const rows = await db.query(uid).findMany({where, select: ['id']}); count = rows.length;`
55. **express-rate-limit НЕ работает в Strapi 5 (Koa)** — пакет
    `express-rate-limit` создаёт middleware с сигнатурой `(req, res, next)`,
    а Strapi 5 использует Koa `(ctx, next)`. Симптом: rate-limit middleware
    не блокирует запросы, но и не считает лимиты (мёртвый код). Заменён на
    чистый Koa middleware с in-memory Map + sliding window.
56. **Rate-limit пропускает OPTIONS (CORS preflight)** — `ctx.method === 'OPTIONS'`
    должен return next() без подсчёта. Иначе браузерные preflight запросы
    (которые идут каждые 3 сек при polling) съедают лимит за минуту.
57. **resp.json() возвращает `unknown`** — в TypeScript strict mode `fetch().json()`
    возвращает `unknown`, не `any`. Нужен явный `as any[]` или `as Record<string, any>`.
    Symptom: `error TS18046: 'results' is of type 'unknown'`.
58. **FocusFilters.priceFrom — string из input** — Vue `<input type="number">` даёт
    `string`, не `number`. Интерфейс `FocusFilters` должен принимать `string | number | null`.
59. **Playwright acceptance не запускает dev-server** — `app/playwright.config.ts`
    больше не стартует Vite/dev/preview. Multi-user spec требует явные
    `SMOKE_UI_URL`, `SMOKE_API_URL`, три разные fixture credentials и
    `E2E_MULTIUSER=1`; production target fail-closed без отдельного explicit
    override. Запуск и полный fixture contract описаны в
    [docs/multiuser.md](multiuser.md), §6 C3.
60. **Seeder перезаписывает пароль test user** — `seedTestUser` в bootstrap
    обновляет пароль из `TEST_USER_PASSWORD` env при каждом старте API.
    Если нужно сменить пароль — менять в `.env` и рестартить API.
61. **Terminal tool маскирует пароли (`***`)** — когда агент передаёт пароль
    через переменную окружения (например `TEST_USER_PASSWORD='xxx' npx playwright test`),
    инструмент terminal автоматически заменяет значение на `***` в выводе.
    Playwright получает `***` вместо реального пароля → тесты падают на auth.
    **Workaround**: записать пароль в файл (`echo "$PASSWORD" > /tmp/.e2e_password`)
    и читать из файла: `TEST_USER_PASSWORD=$(cat /tmp/.e2e_password)`.
    Альтернатива: задать в `.env` и не передавать через shell inline.

62. **API token не работает с `config: {}` (auth required)** — Strapi 5 routes
    без явного `config` или с `config: {}` требуют JWT. API-токен (из `.env`)
    не проходит auth на таких endpoints -> `ForbiddenError: 500`.
    Старое single-tenant решение `auth: false, policies: []` для data routes
    **запрещено** после multi-user wave. Custom routes используют JWT через
    `global::authenticated-user`, а global controls дополнительно
    `global::aklab-admin`; service token разрешён только dedicated internal routes.
    Gotcha #32/#24 (JWT протухает после restart) + этот = причина почему
    `config: {}` не работает в single-tenant.
63. **Analyzer deviation: отрицательная != недооценка** — формула:
    `deviation = (refPrice - actualPrice) / refPrice * 100`.
    deviation > 0 = объект ДЕШЕВЛЕ рынка (недооценён).
    deviation < 0 = объект ДОРОЖЕ рынка (переоценён).
    Старый баг: `isUndervalued = deviation >= threshold` — отрицательная
    deviation (-24%) считалась недооценкой. Фикс: `deviation > 0 && deviation >= threshold`.
64. **Analyzer обнулял deviation_percent для не-undervalued** —
    `deviation_percent: isUndervalued ? deviation : 0` терял реальное
    значение -> focus engine не мог считать скор. Фикс: всегда сохранять
    реальную deviation.


65. **preFilterProperty — фильтрация ДО fetchDetails** —
    Старый flow: parse() → для КАЖДОГО объекта fetchDetails (2-5 сек) → createProperty() (тут фильтры).
    Проблема: 214 детальных загрузок → 29 создано (86% зря).
    Новый flow: parse() → preFilterProperty() (city, stop words, area, price) → только прошедшие → fetchDetails → createProperty.
    Экономия: вместо 291 fetchDetails → 15. Реализовано в .
66. **SettingsView cities.length < 3 пропускал фильтр** —
     — при выборе ВСЕХ 3 городов
    (Москва + МО + Другие) фильтр НЕ передавался → pipeline брал глобальные настройки .
    Тот же баг что #64 в ParseLaunchPanel, но в SettingsView не был исправлен.
    Фикс: .
67. **Pipeline controller логирует фильтры** —
     — теперь видно
    какие фильтры реально дошли до pipeline. Помогает дебажить проблемы с городами/ценами.

65. **preFilterProperty — фильтрация ДО fetchDetails** —
    Старый flow: parse() → для КАЖДОГО объекта fetchDetails (2-5 сек) → createProperty() (тут фильтры).
    Проблема: 214 детальных загрузок → 29 создано (86% зря).
    Новый flow: parse() → preFilterProperty() (city, stop words, area, price) → только прошедшие → fetchDetails → createProperty.
    Экономия: вместо 291 fetchDetails → 15. Реализовано в services/_shared/src/parse-handler.ts.
66. **SettingsView cities.length < 3 пропускал фильтр** —
    Если выбрать ВСЕ 3 города (Москва + МО + Другие), фильтр НЕ передавался → pipeline брал глобальные настройки.
    Тот же баг что и в ParseLaunchPanel, но в SettingsView не был исправлен.
    Фикс: убрано условие length < 3.
67. **Pipeline controller логирует фильтры** —
    Теперь видно какие фильтры реально дошли до pipeline. Помогает дебажить проблемы с городами/ценами.

62. **Двуфазный парсинг** (v1.1.37) — parse-handler разделён на:
    - Phase 1 (scan): парсинг списков + дедуп + предфильтр → сохраняет в `/tmp/aklab-scan/`
    - Phase 2 (details): чтение файла + fetchDetails + createProperty
    - Pipeline синхронизирует фазы глобально: Phase 2 начинается ТОЛЬКО после завершения ВСЕХ Phase 1.
    - Счётчики НЕ прыгают — pipeline читает их только после завершения фазы.
    - Новый stage `parsing_scan_done` между Phase 1 и Phase 2.
63. **resetSourceDetailsCounters** — вызывается ТОЛЬКО в Phase 1 (scan). Phase 2 НЕ сбрасывает счётчики.
64. **total_details_needed** — устанавливается в Phase 1. Если у парсера нет `fetchDetails` — будет 0.

65. **invest-mosreg: цена в млн.руб.** — API invest.mosreg.ru возвращает кадастровую стоимость в млн (31.68 = 31 680 000₽). Парсер конвертирует: `num < 1000 → num * 1_000_000`. Площадь земельных участков в гектарах → `num < 100 → num * 10_000`.
66. **m-ets: auction_type='bankruptcy'** — m-ets.ru это аукционы банкротств, НЕ маркетплейс. `auction_type='marketplace'` обходил `isCommercialProperty()` → автомобили и права требования проходили фильтр.
67. **localStorage фильтры автоматически применяются** — `usePropertyFilters` сохраняет city, property_type, source, price в `localStorage` (key `aklab-property-filters`). При открытии «Все объекты» фильтры из localStorage применяются к запросу. Симптом: «показывает 1 объект вместо 143». Диагностика: `pm2 logs` →смотреть query params в GET /api/properties. Решение: нажать «Сбросить» в фильтрах.
68. **VITE_API_URL вшивается при сборке** — `vite build` берёт `VITE_*` переменные из `app/.env.local` (НЕ из корневого `.env`). Если `.env.local` устарел или не сгенерирован → фронт ходит на неверный API. `deploy-prod.sh` и `deploy-dev.sh` делают `grep -E "^VITE_" .env > app/.env.local` перед сборкой. **НИКОГДА не деплоить вручную** — пропуск этого шага = фронт на prod ходит на prod API из dev.
69. **Object.assign перезаписывает Phase 1 данные** (v1.1.58) — `fetchDetails` возвращает `{ address: undefined, description: undefined }` → `Object.assign(prop, details)` убивает Phase 1 данные. FIX: мерждить только non-null/non-undefined. В `parse-handler.ts`.
70. **jQuery UI табы удалены с etprf** (v1.1.58) — сайт etprf убрал `#tabPage13301/13305/13303`. Все `.details-table` видны одновременно. FIX: искать поля во ВСЕХ таблицах.
71. **etprf "Почтовый адрес" ≠ адрес объекта** — "Почтовый адрес" = адрес организатора (банк). Реальный адрес в "Сведения об имуществе". FIX: искать в описании, НЕ на всей странице.
72. **Regex падежи** — "адрес" не матчит "адресу:" (дательный падеж). FIX: `адрес\w*` везде где есть regex адреса.
73. **SPA async hydration** — alfalot fetchDetails читает DOM до заполнения `.address` и `<p>`. FIX: waitForSelector('.address, h3') + пауза 1s.
74. **m-ets multi-lot price bug** — `.lot-cost-item.price .value` берёт первый лот на multi-lot странице. FIX: `meta[itemprop="price"]` как primary.
75. **Глобальные stop words опасны** — "на право заключения" фильтровал 100% sberbank-ast лотов. Stop words должны быть source-specific или проверяться на реальных данных. FIX: убран из глобальных через SQL.
76. **Playwright browser leak в finally** (v1.1.58) — `sharedBrowser.close()` был ПОСЛЕ цикла for, не в finally. При исключении в Phase 2 — browser не закрывался → zombie chrome-headless-shell процессы растут после каждого pipeline run. На проде 211 процессов = 6.1GB из 7.4GB RAM. **FIX:** `try { for loop } finally { browser.close() }`. **Правило:** ЛЮБОЙ Playwright browser.close() ВСЕГДА в finally блоке. Проверять `pgrep -c -f chrome-headless-shell` на проде — должно быть 0 между запусками. **SUPERSEDED by v1.1.59** — теперь используется sharedContext (см. #77).

77. **Playwright page leak — sharedContext vs sharedBrowser** (v1.1.59) — v1.1.58 закрывал browser в finally, но КАЖДЫЙ `fetchDetails` вызов создавал новый page+context из browser → 1545 pages+contexts никогда не закрывались → 198 zombie chrome процессов → 7GB RAM → OOM. **FIX:** parse-handler создаёт `sharedContext` (BrowserContext) один раз для Phase 2, передаёт его вместо browser. Каждый парсер: `const page = await sharedContext.newPage()` → `finally { page.close() }`. Если standalone (нет sharedContext) — парсер создаёт свой browser+context и закрывает оба. **Правило:** page.close() ВСЕГДА в finally после fetchDetails. context.close() ВСЕГДА в finally parse-handler.

78. **pipeline:daily вместо per-source crons** (v1.1.59) — удалено 10 per-source parse crons (Source.schedule), analyze cron (08:00), digest:morning cron. Один `pipeline:daily` — проверяет каждый час, запускается в digest_time из settings, mode='full' (parseAll → analyze → digest). `rescheduleSource` — no-op. Причина: per-source crons не синхронизировали фазы (Phase 1/2), digest мог прийти до завершения парсинга всех источников.

79. **Photo-fetcher SSRF: DNS rebinding остаётся P0 без pinned egress** (2026-07-16) — URL/detail host, public-IP DNS, HTTPS, redirects, Service Worker и image MIME/size проверяются, но Playwright и Node `fetch` заново резолвят hostname при соединении. Проверка до соединения не защищает от DNS rebinding. **Не деплоить photo-fetcher как закрытую SSRF-защиту** до локального egress-proxy/transport с pinned DNS и CIDR-проверкой на фактическом socket. Пользователь решил оставить компонент без deployment и продолжить остальные фазы.

80. **torgi.gov.ru: HTTP 200 может быть SPA-страницей 404** (v1.1.74) — старый публичный маршрут `/new/public/lots/reg/lot-card/{noticeNumber}/{lotNumber}` отвечает HTTP 200, но после hydration показывает «404. Такой страницы не существует». Актуальный маршрут: `/new/public/lots/lot/{noticeNumber}_{lotNumber}`; compound ID совпадает с `item.id` и detail API `/new/api/public/lotcards/{noticeNumber}_{lotNumber}`. **Правило:** ссылки SPA проверять по реальному DOM/body, а не только по HTTP status. При смене маршрута исправлять и генерацию новых URL, и существующие записи БД после backup + transaction + integrity check.

81. **torgi.gov.ru photos: API-first, original image endpoint, Russian CA** (v1.1.75) — Playwright/Chromium падает на Torgi с `net::ERR_CERT_AUTHORITY_INVALID`, потому что `NODE_EXTRA_CA_CERTS` применяется к Node, но не к Chromium. Для фото использовать lot API `/new/api/public/lotcards/{compoundId}` → `lotImages[]` → `/new/image-preview/v1/{fileId}?disposition=inline`; file ID принимать только как 24 hex chars, максимум 20. **Не использовать `resize=600x600!`**: наблюдались 503 и ответ `Content-Type: image/png` с JPEG magic bytes, который строгая проверка обязана отклонять. Worker запускается с Russian CA chain, TLS verification не отключается; временные 429/5xx ретраятся с backoff, полный провал должен бросать ошибку для retry очереди.

82. **GNU `ln -sf` следует symlink-to-directory** (2026-07-19) — повторный deploy выполнял `ln -sf target api/node_modules/@aklab/parse-rules`; если destination уже был symlink на директорию, GNU ln создавал вложенную сломанную ссылку `lib/parse-rules/parse-rules`. Использовать `ln -sfn`, где `-n` запрещает dereference destination. После deploy проверять `git status --porcelain` и отсутствие вложенной ссылки. `npm install` также может менять только root version в `package-lock.json`; такой diff синхронизировать отдельным PR, не делать слепой `git checkout -- .` при наличии production backup/diagnostics.

83. **Multi-user migration — только explicit absolute paths и dev gate** — `api/scripts/migrate-multiuser.js` в audit требует `--db=/absolute/...`, а apply дополнительно требует отдельный `--backup=/absolute/...`; implicit DB path, relative backup, Strapi bootstrap и server runtime запрещены. Strapi SQLite может создать relation tables с duplicate-identical FK metadata и не материализовать field-level `unique`: audit тогда показывает `schema.ready=false` только по четырём explicit unique flags, а apply после verified backup транзакционно создаёт named UNIQUE indexes и обязан завершиться с `after.schema.ready=true`. Missing/ambiguous relations и duplicate values остаются fail-closed. Порядок `feature OFF → audit → transactional migration → idempotent re-audit → private photo copy → dev-only flag ON` и rollback triggers записан в [docs/multiuser.md](multiuser.md). `MULTIUSER_ENABLED` включается только exact raw `true`; legacy rows не удалять, private photo originals сохранять до отдельной cleanup wave.

84. **Dev PM2 daemon shared с TODOIT — не делать automatic `pm2 update`** —
    `pm2 update` перезапускает весь daemon, а на `192.168.11.151` он обслуживает
    одновременно AKLAB и TODOIT. Dev deploy при Node-version mismatch обязан
    остановиться до build/restart и запросить отдельное infrastructure решение;
    нельзя автоматически менять daemon в AKLAB deploy script.

85. **Route-only Strapi namespace не является content type** — controller для
    custom internal routes без `content-types/*/schema.json` экспортировать plain
    object с actions. `factories.createCoreController(uid, ...)` требует реально
    зарегистрированный UID и при route composition читает `contentType.kind`;
    фиктивный UID приводит к startup crash до открытия HTTP-порта. Contract test
    обязан проверять object-shaped default export, а dev acceptance — реальный
    Strapi health после route registration.

86. **Failed checkout не равен last-known-good rollback SHA** — fast-forward и
    clean Git доказывают только source identity, но не successful runtime. Dev
    deploy принимает explicit `--rollback-ref` и хранит last-success SHA вне
    checkout только после core/service health gates. Health failure должен идти
    через `ERR` trap; прямой `exit 1` внутри gate обходит trap Bash.

87. **Focus threshold — endpoint-specific query нельзя пропускать через обычный catalog sanitizer** — `buildPropertyQuery()` намеренно оставляет только общие ключи и удаляет `threshold`. Если `PropertyFocusTab` передаст слайдер через него, `GET /properties/focus` получит threshold по умолчанию `0` и покажет все объекты. Использовать `buildFocusPropertyQuery()`; regression test должен проверять, что `threshold` доходит до `/properties/focus`.

88. **Multi-user parser reads нельзя оставлять на пользовательских/admin endpoints** — после закрытия generic `GET /properties` и `GET /sources/:id` service token получает `403`. `propertyExists()` работает fail-closed и тогда считает все проверенные кандидаты существующими: pipeline формально завершается, но создаёт 0 объектов, а digest получает `empty`. Parser reads должны идти только через минимальные service-token aliases: identity boolean `GET /internal/properties/exists` и allowlisted counters `GET /internal/sources/:id/stats`. Не открывать generic routes для service token. Regression обязан проверять URL клиента, строгий DTO, route policy и fail-closed malformed/non-2xx.

89. **Digest window нельзя брать из filter snapshot, созданного до parsing** — `filter_snapshot.windowEndAt` фиксирует входной профиль и время старта run. Объекты, созданные этим же pipeline, имеют `first_seen_at` позже и выпадают из `(lower, upper]`, поэтому digest может вернуть `empty` при ненулевых `created` и `focus_score`. Фиксировать отдельный first-write-wins `parser_run.digest_window_end_at` непосредственно перед fan-out; projection должна читать только его, проверять canonical ISO и `digest_window_end_at >= filter_snapshot.windowEndAt`. Snapshot не мутировать и hash не пересчитывать.

90. **Digest projection не должна возвращать общий UI DTO** — `UserPropertyDto` содержит поля, которых нет в fail-closed allowlist digest worker. Если вернуть его напрямую, реальные non-empty rows дадут `Digest projection response is invalid`, хотя empty fixture/tests проходят. Делать явный mapping в exact digest DTO и regression с fixture, содержащей UI-only поля; проверять полный sorted key set, а не только отсутствие одного PII-поля.
91. **Десятичная точка в ценах торговых площадок — не разделитель тысяч** — M-ETS metadata и XML Сбербанк-АСТ отдают `615150.00`, `10204.43` и `3796542.4`. Regex, который оставляет только цифры и запятую, превращает их в `61515000`, `1020443`, `37965424`. Все текстовые parser prices должны использовать `parsePrice()` из `@aklab/service-shared`; он различает `1 500 000,00`, `1.500.000,00`, `1.500.000` и dot-decimal. Добавлять regression на подтверждённый source format до изменения parser logic.

92. **Явный UTC/offset deadline нельзя интерпретировать как московское локальное время** — API ГИС Торги отдаёт `biddEndTime` в ISO с `Z` (например, `2026-08-21T08:00:00Z`), а Invest Moscow — ISO с семизначной дробной частью секунд. `parseAuctionEndAt()` должен сохранять такое значение через `Date.parse()` в canonical UTC ISO; только timezone-less русские/ISO даты нормализуются как МСК. TorgiGovParser обязан брать именно `biddEndTime`, а не подменять deadline датой начала аукциона.

93. **Parser drift нельзя маскировать coverage fallback-ом** — geography извлекается только из bounded current-property fields; organizer/debtor/pledgee/customer addresses, `body.innerText`, title, excerpt и «первый адрес» запрещены. Detail result с итоговым `property_location.status=missing` не persist'ится: это `location_unresolved`; detail exception остаётся failure и не использует stale scan geography. DOM/API shape наблюдается только allowlisted semantic IDs + SHA-256 fingerprint, без raw HTML/PII. Canary использует отдельный exact `operation=probe`, 1–3 samples, общий pipeline lifecycle lock, один orchestrated cron и no Property/analyze/digest path. Drift recovery: sanitized fixture → RED → минимальный source adapter fix → focused/full tests → independent exact diff/SHA review → отдельное approval для push/PR/deploy/reparse/cleanup. Подробно: [parser contracts](parser-source-contracts.md), [telemetry](run-scoped-parser-telemetry.md), [runbook](parser-drift-runbook.md).

94. **Одна transient detail-карточка не должна повторять весь immutable source artifact** — pipeline snapshot может содержать сотни карточек, и детерминированный timeout/ошибка одной карточки иначе обрывает источник и трижды повторяет уже успешные details. Обычное исключение непосредственно из `fetchDetails()` учитывается как item-level `failed + skipped`, source telemetry завершается `degraded/parser.transient`, а остальные карточки продолжают обрабатываться. Typed source/quarantine/cancellation/location errors, ошибки после `fetchDetails()` и persistence остаются fail-closed. Если `details_ok=0` и упали все detail requests, stage остаётся failed/retryable и сохраняет artifact. Queue reconciliation обязана сохранять allowlisted `parser.<class>`, а не hardcode `permanent`.

95. **Queue terminal health и cancellation требуют разных путей** — после reconciliation source health пересчитывается для terminal `completed` и `failed` details jobs, чтобы zero-detail/typed failures не оставляли устаревший `healthy` gate. Job с `cancellation_requested_at` health не меняет: операторская отмена не является source degradation. Поэтому все cancellation entrypoints (`requestCancellation`, `cancelJob`, `clearQueue`) обязаны атомарно сохранять `cancellation_requested_at` и для pending terminal jobs, а не полагаться на текст `error='cancelled'`. При `cancelled=true` terminal telemetry всегда хранит `parser.cancelled`, даже если stale worker error содержал другой allowlisted class.

96. **PM2 memory recycle выглядит как queue `stale job recovery`** — длинный `aggregator-bankrot` details job может превысить общий parser ceiling `512M`. PM2 перезапускает worker, active queue row остаётся без владельца и затем штатно подбирается как новый attempt с `error='stale job recovery'`; это не detail-fetch error и не cancellation. Нельзя лечить только runtime-командой PM2: следующий immutable deploy восстановит ecosystem. `services/services.json` задаёт для aggregator durable `max_memory_restart: "1024M"`, а `ecosystem.config.js` использует per-parser override с fallback `512M`. Regression обязан подтверждать `1024M` только для aggregator и `512M` для остальных parsers. При диагностике сравнивать queue attempts/error с ростом `pm2_env.restart_time` и ceiling, не менять queue вручную.

97. **`page.evaluate()` не видит imported Node helpers** — TypeScript может успешно собрать browser callback, содержащий вызов `parseAuctionEndAt()` или другого импорта, но в Chromium closure это становится ссылкой вроде `service_shared_1`, которой нет в page context. Симптом: все detail items падают `parser.transient`, а live probe показывает `page.evaluate: ReferenceError: service_shared_1 is not defined`. Browser callback должен возвращать только raw serializable payload; imported normalization/parsing выполняется после `await page.evaluate(...)` в Node. Regression должен проверять отсутствие imported helper call внутри evaluate closure. Не ослаблять all-details-failed fail-closed contract.

98. **Torgi transient request не должен перезапускать весь scan с нуля** — официальный API периодически отвечает `429/5xx` или даёт network timeout; один такой ответ раньше немедленно валил source, после чего queue до трёх раз повторяла все search queries/pages целиком. Production run `64bc8fe0-af60-4163-9c45-12eb639a5b98` показал, что короткие `3 attempts` с `1s/2s` восстанавливают одиночные 503, но после сотен страниц API входит в sustained throttle window: каждая queue retry снова разогревает лимит и заканчивается `parser.transient`. Для list и detail API применяется адаптивный bounded profile: максимум 5 attempts, 30s timeout на attempt, backoff `5s/15s/30s/60s`, numeric `Retry-After` (cap 120s) и дополнительный cooldown `15s` после успешного восстановления. Response body отменяется перед retry. `429/5xx/network` ретраятся, но `401/403/451` и прочие permanent 4xx возвращаются сразу и остаются fail-closed через `parserHttpError`. TLS не отключать: production parser обязан иметь существующий `NODE_EXTRA_CA_CERTS=/usr/local/share/ca-certificates/russian-ca-chain.pem`; проверять только boolean/path existence и live HTTP metadata, не выводить env secrets.

100. **`lib/sqlite-queue` артефакты (.js/.d.ts/.js.map) отслеживаются git — пересобирать до деплоя** — если в сессии менялись `.ts` файлы `lib/sqlite-queue` (worker.ts, index.ts), нужно пересобрать (`cd lib/sqlite-queue && npm run build`) и закоммитить артефакты до `git push origin main`. Иначе `deploy-prod.sh` сам пересоберёт их на сервере, получит dirty worktree и упадёт с `ERROR: Deploy изменил tracked worktree; refusing to leave production Git dirty`. Первый деплой завершается полностью (PM2 online, health OK), но финальная проверка чистоты падает → следующий деплой тоже падает на preflight из-за оставшегося грязного worktree. Лечение: `git checkout -- lib/sqlite-queue/` на сервере, затем новый деплой с правильным полным SHA.

101. **`deploy-prod.sh --ref` требует полный 40-символьный SHA** — сокращённый SHA (8 символов) даёт `ERROR: origin/main is <full-sha>, expected release <short>; refusing a raced deploy`. Всегда передавать вывод `git rev-parse origin/main`.

102. **Fabrikant multi-lot — first `lot_delivery_place` ≠ текущий лот** — на странице процедуры несколько `#lot-*` блоков с отдельными `.panel-group-element-lot_delivery_place`. Scan с keyword-filter по title пропускает лоты с названием ООО; `fetchDetails` через `querySelector`/`body.innerText` брал первый лот (Москва) при брянском title. FIX (v1.1.96): expand процедуры в `parse()`, lot-view URL, extraction только из `#lot-{lotId}`; без lotId при >1 delivery_place — fail-closed `missing`. См. `docs/planlot.md`, `services/parser-fabrikant/src/sources/fabrikant-extraction.ts`.

103. **Multi-lot HTML без scoped identity — fail-closed, не merge** — если detail-страница содержит ≥2 property-блока с разными регионами/ценами/описаниями, а текущий лот не выделен (`lotId` в URL, `#lot-*`, `data-lot-id`), парсер обязан вернуть `property_location.status=missing` (+ `schema_mismatch`) или skip, но **никогда** не смешивать first/last `querySelector` в одну Property. Title не география. См. инцидент Fabrikant (`docs/planlot.md`) и аудит (`docs/plan-lots-audit.md`). Regression: sanitized 2-lot fixture + assert отсутствия mixed row (`services/_shared/src/__tests__/multi-lot-assertions.ts`).

104. **М-ЕТС multi-lot — slug URL и itemscope price, не `/lot/{id}`** — listing API даёт `href` вида `228875-1`, detail URL `https://m-ets.ru/228875-1` (не `/lot/268741965`). На multi-lot trade на lot-URL рендерятся **2** блока `.lot-info-block.info-type_1` без `data-lot-id`; текущий лот — блок внутри `[itemscope]`, где есть единственный `meta[itemprop=price]`. FIX (v1.1.97): `findMetsPropertyBlock` выбирает priced block; fail-closed если priced ≠ 1. Live probe (`scripts/probe-multi-lot-live.js`) должен брать URL из `href` карточки API, не из `lot_id`. Не использовать first `querySelector` по всему документу.

99. **Legacy `filter_rent=NULL` — это compatibility default, а не malformed profile** — после добавления profile-scoped фильтра аренды старые строки могут физически не иметь нового Boolean. Immutable snapshot должен трактовать и `undefined`, и `null` как effective `true`; иначе manual full run завершится до scan typed `USER_PROFILE_MALFORMED`. Не лечить raw DB backfill или profile-policy bypass: сначала RED test на nullable stored field, затем узкий normalizer fix, release и новый run. Подтверждено production rebuild 2026-08-15; hotfix v1.1.95.

## Runtime hardening contracts (2026-08-21)

105. **Queue provenance — explicit origin, no generic per-job table** — pipeline analyzer jobs use `origin=pipeline, runId, stage=analyze`; canary probes use the exact `operation=probe, origin=canary, runId, stage=probe` envelope; user lazy photo jobs use `origin=user, stage=photo_fetch` and never receive a fake `runId`. Analyzer/photo workers accept legacy payloads only when all provenance fields are absent; partial or malformed envelopes fail before side effects.

106. **Canary budget and lifecycle ownership are cooperative** — default/max probe timeout is 120 seconds. Do not use `Promise.race` or detached browser cleanup. If the acknowledgement cap finds an active owned job, do not write Source health or release to `idle`; retain `cancelling/canary` and the exact job IDs until terminal acknowledgement.

107. **No-observation details rows are not healthy evidence** — a detail-capable `success`/`success_empty` row with `details_attempted=0`, no semantic fingerprint and no error returns no classification and performs no baseline, Source or annotation write. Persisted counters are strict non-negative safe integers; malformed telemetry fails closed.

108. **Absolute parser-health floors precede baseline deltas** — with sample size >=20, detail success below 0.8 is `degraded.details_ok_ratio_below_floor`, and missing location above 0.2 is `degraded.location_missing_ratio_above_ceiling`. Sample 19 is not subject to these absolute floors. `degraded.canary_no_samples` is a separate reason. A terminal degraded details row is a floor; healthy canary evidence is non-promoting, while informative healthy full-details evidence may recover a non-hard degraded state.

109. **Canary and parser drift boundaries remain typed and fail-closed** — an unhydrated property container is distinct from a hydrated container without allowlisted property-location labels. The first remains transient; the second yields typed `missing`. No body/title/party-address fallback is allowed. This runtime-hardening wave does not change digest, MarketReference, focus rules, settings, frontend, deploy, or catalog cleanup behavior.

110. **Parser telemetry `failed` и `skipped` взаимоисключающие** — detail adapter exception увеличивает только `failed`; `skipped` означает успешно оценённый, но не сохранённый кандидат. Для detail-capable terminal snapshot: `details_ok + failed = details_attempted` и `created + skipped + failed = details_attempted`. Completed source health `degraded/schema_changed/blocked` обязан переводить parent parser run в `degraded`/`done_with_errors`, но не создавать queue retry. Legacy overlapping rows не переписывать.

111. **Roseltorg HTTP 200 не доказывает parser contract** — production probe 2026-08-22 получил нормальный HTTP 200 SPA shell без Qrator/block markers, но raw markup не содержал текущих table/card selectors; последние 18 запусков дали 0 cards. Текущий bounded root cause — markup/hydration contract failure, не подтверждённый WAF. Нужен source-faithful hydrated DOM/API probe; не внедрять proxy/tunnel без свежего block evidence.

112. **MarketReference enum должен совпадать с Property** — наличие rows в БД недостаточно: content-type schema и internal lookup allowlist обязаны принимать те же canonical property types, включая `apartment` и `land`. Сначала release schema/controller, затем authenticated Admin create + internal lookup read-back; raw SQL не использовать. Утверждённый baseline 2026-08-22: `docs/market-reference-baseline-2026-08-22.md`.
